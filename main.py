...
def authenticate():
    try:
        ...
        # Check the label and proceed with face recognition
        if label == 1:
            name = util.recognize(img, './db')
            if name in ['unknown_person', 'no_persons_found']:
                return jsonify({"status": "fail", "message": "Unknown user"}), 401
            else:
                return jsonify({"status": "success", "name": name}), 200
        else:
            return jsonify({"status": "fail", "message": "Spoofing detected"}), 403
